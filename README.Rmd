...
title: 'Road traffic fatalities by light condition: a comparative analysis between the US and the UK'
...
Table 1. Road traffic fatalities by light condition and type, US 2011-2020
...
data_table <- fars %>% 
  select(A_PTYPE, YEAR, LGT_COND, A_PERINJ, `Light condition in detail` = LGT_CONDNAME) %>% 
  mutate(`Light condition in detail` = factor(`Light condition in detail`, levels = c("Daylight", "Dark - Lighted", "Dark - Not Lighted", "Dark - Unknown Lighting", "Dawn", "Dusk"))) %>% 
  ...
  filter(A_PERINJ == 1 & `Light condition in detail` != "Unkown") %>%
    group_by(Type, `Light condition in detail`) %>% 
  summarise(`Num.` = n()) %>% 
  mutate(`%` = round(`Num.`/sum(`Num.`)*100, 2)) %>%
  mutate(`Light condition` = ifelse(`Light condition in detail` == "Daylight", "Daylight", "Dark")) %>% 
  select(Type, `Light condition`, `Light condition in detail`, `Num.`, `%`)
...
data_plots = fars %>% 
  ...
  mutate(`Light condition` = ifelse(LGT_COND == 1, "Daylight", ifelse(LGT_COND >= 1 & LGT_COND <= 6, "Dark", "Unkown"))) %>% 
  mutate(`Light condition` = factor(`Light condition`, levels = c("Daylight", "Dark", "Unkown"), labels = c("Daylight", "Dark", "Unkown"))) %>% 
  mutate(Type = ifelse(A_PTYPE == 3, "Pedestrians", ifelse(A_PTYPE == 4, "Cyclists", "Others"))) %>%
  filter(A_PERINJ == 1, `Light condition` != "Unkown") %>%
  group_by(YEAR, Type, `Light condition`) %>% 
  summarise(n = n()) %>% 
  mutate(perc = n/sum(n)*100)
...
p4_us = data_plots %>% 
  filter(Type == "Pedestrians") %>% 
  ggplot() +
  geom_col(aes(YEAR, y = perc, fill = `Light condition`, colour = `Light condition`), position = "dodge", alpha = 0.7) + 
  labs(title = "Pedestrians",
       x = "", 
       y = "% of fatalities", 
...
Figure 2. % of road traffic fatalities by light condition, year, and type, US 2011-2020 
...
Table 2. Road traffic fatalities by light condition and type, UK 2011-2020
...
data_table <- cva11_20%>% 
  select(casualty_type, YEAR = accident_year, casualty_severity, `Light condition in detail` = light_conditions) %>% 
  mutate(`Light condition in detail` = factor(`Light condition in detail`, levels = c("Daylight", "Darkness - lights lit", "Darkness - lights unlit", "Darkness - no lighting", "Darkness - lighting unknown", "Data missing or out of range"))) %>%
  ...
  mutate(Type = factor(Type, levels = c("Pedestrians", "Cyclists", "Motorised vehicles occupants", "Others"))) %>% # Criteria on Motorised vehicles occupants category is not the same in the UK and US. Explain in method
  filter(casualty_severity == "Fatal" & Type != "Others" & `Light condition in detail` != "Data missing or out of range") %>%
    group_by(Type, `Light condition in detail`) %>% 
  summarise(`Num.` = n()) %>% 
  mutate(`%` = round(`Num.`/(sum(`Num.`))*100,2))  %>%
  mutate(`Light condition` = ifelse(`Light condition in detail` == "Daylight", "Daylight", "Dark")) %>% 
...
data_plots <- cva11_20%>% 
  select(casualty_type, YEAR = accident_year, casualty_severity, `Light condition in detail` = light_conditions) %>% 
  mutate(`Light condition` = ifelse(`Light condition in detail` == "Darkness - lights lit" | `Light condition in detail` == "Darkness - lights unlit" | `Light condition in detail` == "Darkness - no lighting" | `Light condition in detail` == "Darkness - lighting unknown", "Dark", ifelse(`Light condition in detail` == "Data missing or out of range", "NA", "Daylight"))) %>%
 mutate(`Light condition` = factor(`Light condition`, levels = c("Daylight", "Dark"), labels = c("Daylight", "Dark"))) %>% 
  mutate(Type = ifelse(casualty_type == "Pedestrian", "Pedestrians", ifelse(casualty_type == "Cyclist", "Cyclists", ifelse(casualty_type == "Horse rider", "Others", "Motorised vehicles occupants")))) %>%
  mutate(Type = factor(Type, levels = c("Pedestrians", "Cyclists", "Motorised vehicles occupants"))) %>%
  filter(casualty_severity == "Fatal" & Type != "Others" & `Light condition in detail` != "Data missing or out of range") %>%
  group_by(YEAR, Type, `Light condition`) %>% 
...
Figure 4. UK % of road traffic fatalities by light condition, year, and type, UK 2011-2020

# Next steps

- Add a total row per type of fatality in each table.

- Estimate risk indices. Get denominator (distance, time, etc) through the NHTS and NTS. Tricky step as we need to calculate veh-km by light condition (daylight/dark). I suppose we need to create function to make this calculation. Problems here: not all trips will be A or B, different hour times in the US, etc. 

- Compare cities?

- Check association of risk indices with the sale of SUVs, community severance (pedestrian crossings length and density), alcohol consumption, gender, ethnicity, income, etc. Safety in numbers is also interesting: less walking may mean higher risk.
